...
class ClassBlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio, norm_layer=nn.LayerNorm):
        ...
        self.attn = MSS(dim=dim,dim_expansion_factor = 2)
...
@register_model
def mongoose_s(pretrained=False, **kwargs):
    model = Mongoose(
        stem_hidden_dim = 32,
        embed_dims = [64, 128, 320, 448], 
        num_heads = [2, 4, 10, 14], 
...
@register_model
def mongoose_b(pretrained=False, **kwargs):
    model = Mongoose(
        stem_hidden_dim = 64,
...
@register_model
def mongoose_l(pretrained=False, **kwargs):
    model = Mongoose(
        stem_hidden_dim = 64,
